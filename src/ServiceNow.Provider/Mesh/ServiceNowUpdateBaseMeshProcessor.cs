...
{
    public abstract class ServiceNowUpdateBaseMeshProcessor : BaseMeshProcessor
    {
        public EntityType[] EntityType { get; }
        public string EditUrl { get; }

        protected ServiceNowUpdateBaseMeshProcessor(ApplicationContext appContext, string editUrl, params EntityType[] entityType)
            : base(appContext)
        {
        ...
        public override List<RawQuery> GetRawQueries(IDictionary<string, object> config, IEntity entity, Core.Mesh.Properties properties)
        {
            ...
            return new List<Core.Messages.WebApp.RawQuery>()
            {
                ...
                {
                    Query = string.Format("curl -X PUT " + string.Format("{0}/api/now/table/", serviceNowCrawlJobData.BaseUrl) + EditUrl + "--header 'Authorization: Basic {0}' --header \"Content-Type: application/json\"" + " --data '{2}'", serviceNowCrawlJobData.Username, this.GetLookupId(entity), JsonUtility.Serialize(properties)),
                    Source = "cUrl"
                }
            };
        }
        ...
        public override List<QueryResponse> RunQueries(IDictionary<string, object> config, string id, Core.Mesh.Properties properties)
        {
            ...
            var request = new RestRequest(EditUrl, Method.PUT);
